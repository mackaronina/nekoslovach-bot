...
class NewModel(BaseModel):
    ...
    tags: list[Annotated[str, Field(min_length=1, max_length=25)]] = Field(min_length=1, max_length=3)

    def __str__(self) -> str:
        tags = [html.escape(tag.replace(' ', '_').replace('-', '_').lower()) for tag in self.tags]
